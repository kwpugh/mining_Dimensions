...
public class HuntingTeleporter extends Item
{
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand)
    {
        ...
        ServerWorld targetWorld = ((ServerWorld)world).getServer().getWorld(MiningDimsRegistry.MININGDIMS_WORLD_KEY2);

        ServerPlayerEntity serverPlayer = (ServerPlayerEntity) player;

        RegistryKey<World> currentWorldKey = world.getRegistryKey();
        if(currentWorldKey == MiningDimsRegistry.MININGDIMS_WORLD_KEY2)
        {
            destWorld = overWorld;
        }
        else
