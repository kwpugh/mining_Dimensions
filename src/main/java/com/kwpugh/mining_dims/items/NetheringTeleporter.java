...
public class NetheringTeleporter extends Item
{
    public NetheringTeleporter(Item.Settings settings)
    {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand)
    {
        ...
        ServerWorld targetWorld = ((ServerWorld)world).getServer().getWorld(MiningDimsRegistry.MININGDIMS_WORLD_KEY4);

        ServerPlayerEntity serverPlayer = (ServerPlayerEntity) player;

        RegistryKey<World> currentWorldKey = world.getRegistryKey();
        if(currentWorldKey == MiningDimsRegistry.MININGDIMS_WORLD_KEY4)
        {
            destWorld = overWorld;
        }
        else
        ...
        serverPlayer.sendMessage((new TranslatableText("item.mining_dims.teleporter1")), true);
